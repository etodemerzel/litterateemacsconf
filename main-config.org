...
* Введение
  ...
  Так как я в прошлый раз забыл сохранить сформулированный снипет в файл для орг-мода, то он пропал. Плюс до кучи выяснилось, что комбинация C-a C-y C-s уже на уровне C-a занята. Поэтоу теперь снипет со вставкой elisp  кода с танглом в конфигурационный файл, назначена на новую комбинацию C-c C-g.
...
** Имя и адрес электронной почты
   ...
   Эти параметры используются в подписи при автоматической генерации документов(html страницы из орг-документа, например) или для идентификации в джаббер-расширениии для имакса. Которого у меня в прочем нет. Но всё ещё впереди.
...
**** Система счисления кодов символов
     Указания на то, в какой система счисления вводятся символы по команде C-q
     #+begin_src elisp :tangle ~/.emacs
...
*** Конфигурация цветовой схемы(и шрифта) для Omnius
...
**** wcheck + hunspell
     ...
     Скрипт определяет, есть ли в составе входного слова латинска буква, и в случае положительного решения, отправляет на  проверку с английским словарём, а в случае отрицательного, с русским.
...
**** wcheck hotkey's
     ...
     Так как горячие клавиши будут актуальны при любой настройке, вынесу их как элемент поддающийся обощению над мноэеством конфигураций wcheck-mode
